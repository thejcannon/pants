...
impl Store {
  ...
  pub async fn materialize_symlink(
    ...
    symlink(target.clone(), destination.clone())
      .await
      .map_err(|e| {
        StoreError::from(e).enrich(&format!(
          "Failed to symlink from {target:?} to {destination:?}"
        ))
      })?;
    Ok(())
  }
  ...
  pub async fn materialize_hardlink(
    ...
    copy(target.clone(), destination.clone())
      ...
      .map_err(|e| {
        StoreError::from(e).enrich(&format!(
          "Failed to copy from {target:?} to {destination:?}"
        ))
      })?;

    #[cfg(not(target_os = "macos"))]
    {
      //assert!(metadata(target.clone()).await.is_ok());
      hard_link(target.clone(), destination.clone())
        .await
        .map_err(|e| {
          StoreError::from(e).enrich(&format!(
            "Failed to hardlink from {target:?} to {destination:?}"
          ))
        })?;
    }
